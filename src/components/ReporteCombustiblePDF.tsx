...
const generarDatosReporte = (datos: InformeConsumoCombustibleResponse[]) => {
  ...
  const totalGeneral = datos.reduce((sum, factura) => {
    const totalFactura = factura.detalles.reduce(
      (sumDetalle, detalle) => sumDetalle + detalle.total,
      0
    );
  ...
  const totalGalones = datos.reduce((sum, factura) => {
    const totalGalonesFactura = factura.detalles.reduce(
      (sumDetalle, detalle) => sumDetalle + (detalle.cantidad || 0),
      0
    );
  ...
  return {
    ...
    fecha: dayjs(primerRegistro.fecha_emision)
      .tz("America/Lima")
      .format("DD/MM/YYYY HH:mm:ss"),
    almacen: primerRegistro.almacenes || "N/A",
    clienteOP: `${primerRegistro.numero_factura} ${dayjs(
      primerRegistro.fecha_emision
    )
      .tz("America/Lima")
      .format("DD/MM/YYYY")}`,
    referencia: primerRegistro.nombre || "N/A",
    guia_remision: primerRegistro.guia_remision || "N/A",
    consumoCombustible: primerRegistro.alfanum || "N/A",
...
const generarDetalleCombustible = (
...
const ReporteCombustibleDocument = ({
  datos,
}: ReporteCombustibleDocumentProps) => {
...
const ReporteCombustiblePDF: React.FC<ReporteCombustiblePDFProps> = ({
  ...
  const document = useMemo(
    () => <ReporteCombustibleDocument datos={datosReporte} />,
    [datosReporte]
  );
