...
import { camionesApi, type EmpresaData, type CamionData } from "@/lib/connections";

interface EditCamionDialogProps {
  camion: CamionData;
  empresas: EmpresaData[];
  onCamionUpdated?: () => void;
  buttonText?: string;
  buttonVariant?: "default" | "outline" | "ghost" | "link" | "destructive" | "secondary" | null | undefined;
}

export function EditCamionDialog({
  camion,
  empresas,
  onCamionUpdated,
  buttonText = "Editar",
  buttonVariant = "ghost",
}: EditCamionDialogProps) {
  ...
  // Cargar datos del camión cuando se abre el diálogo
  useEffect(() => {
    if (open) {
      setFormData({
        placa: camion.placa || "",
        ...
        capacidad_tanque: camion.capacidad_tanque?.toString() || "",
        dni: camion.dni || "",
        nombre_chofer: camion.nombre_chofer || "",
        apellido_chofer: camion.apellido_chofer || "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await camionesApi.update(camion.id_camion, dataToSend);

      toast.success("Unidad/Maquinaria actualizada exitosamente");
      setOpen(false);
      ...
      console.error("Error al actualizar unidad/maquinaria:", error);
      const errorMessage = error instanceof Error
        ? error.message
        : "Error al actualizar unidad/maquinaria";
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant={buttonVariant} size="sm">
          <Pencil className={buttonText ? "h-4 w-4 mr-2" : "h-4 w-4"} />
          {buttonText}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar Unidad/Maquinaria</DialogTitle>
          <DialogDescription>
            Modifica los datos de la unidad o maquinaria.
          </DialogDescription>
        </DialogHeader>
