...
type MultifacturaRow = {
  ...
  // Archivos pendientes de subir (solo en estado local)
...
export default function OrdenCompraPage() {
  ...
  const [multifacturasRows, setMultifacturasRows] = useState<MultifacturaRow[]>([
                            ...
                            <div className="space-y-3 pt-2">
                              ...
                              <div className="flex gap-2 pt-2 border-t">
                                <Button
                                  ...
                                  className="flex items-center gap-1 bg-white hover:bg-blue-50 text-blue-700 border-blue-300"
                                >
                ...
                <DialogContent className="sm:max-w-[900px] max-h-[90vh] flex flex-col">
                          ...
                          {multifacturasRows.map((row, idx) => (
                            <tr key={idx} className="hover:bg-gray-50">
                              {(["nro_serie", "nro_factura", "galones", "proyecto"] as const).map((field) => (
                                <td key={field} className="border border-gray-300 p-1">
                                  <input
                                    type="text"
                                    value={row[field]}
                                    onChange={(e) => setMultifacturasRows(prev => prev.map((r, i) =>
                                ...
                                {row.url_factura && !row.file_factura ? (
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        input.onchange = (e) => {
                                          ...
                                          if (file) setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, file_factura: file } : r));
                                        };
                                        input.click();
                                      }}
                                      className="text-blue-400 hover:text-blue-600 shrink-0 text-xs px-1 border border-blue-200 rounded"
                                    >
                                      ↺
                                    </button>
                                  </div>
                                  ...
                                  <div className="space-y-1">
                                    <div className="flex items-center gap-1">
                                      ...
                                      <button
                                        onClick={() => setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, file_factura: null } : r))}
                                        className="text-gray-300 hover:text-red-400 shrink-0 text-xs"
                                  ...
                                  <div className="flex items-center gap-1">
                                    ...
                                    <button
                                      onClick={() => setMultifacturasRows(prev => prev.map((r, i) =>
                                        i === idx ? { ...r, file_factura: null } : r
                                      ))}
                                  ...
                                  <div
                                    className={`h-10 border-2 border-dashed rounded flex items-center justify-center text-xs cursor-pointer transition-colors ${
                                      row.dragging_factura
                                        ? "border-teal-400 bg-teal-50 text-teal-600"
                                        : "border-gray-300 text-gray-400 hover:border-teal-300 hover:text-teal-500"
                                    }`}
                                    onDragOver={(e) => { e.preventDefault(); setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, dragging_factura: true } : r)); }}
                                    onDragLeave={() => setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, dragging_factura: false } : r))}
                                    onDrop={(e) => {
                                      e.preventDefault();
                                      const file = e.dataTransfer.files[0];
                                      if (file) setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, file_factura: file, dragging_factura: false } : r));
                                    }}
                                    onClick={() => {
                                      const input = document.createElement("input");
                                      input.type = "file";
                                ...
                                {row.url_guia && !row.file_guia ? (
                                  <div className="flex items-center gap-1">
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        input.onchange = (e) => {
                                          const file = (e.target as HTMLInputElement).files?.[0];
                                          if (file) setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, file_guia: file } : r));
                                        };
                                        input.click();
                                      }}
                                      className="text-blue-400 hover:text-blue-600 shrink-0 text-xs px-1 border border-blue-200 rounded"
                                  ...
                                  <div className="space-y-1">
                                    <div className="flex items-center gap-1">
                                      <span className="text-gray-400 line-through truncate text-xs flex-1">actual</span>
                                      <button
                                        onClick={() => setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, file_guia: null } : r))}
                                        className="text-gray-300 hover:text-red-400 shrink-0 text-xs"
                                        title="Cancelar reemplazo"
                                      >
                                        <X className="h-3 w-3" />
                                  ...
                                  <div
                                    ...
                                    onDragOver={(e) => { e.preventDefault(); setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, dragging_guia: true } : r)); }}
                                    onDragLeave={() => setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, dragging_guia: false } : r))}
                                    onDrop={(e) => {
                                      e.preventDefault();
                                      const file = e.dataTransfer.files[0];
                                      if (file) setMultifacturasRows(prev => prev.map((r, i) => i === idx ? { ...r, file_guia: file, dragging_guia: false } : r));
                                    }}
                                    onClick={() => {
                                      const input = document.createElement("input");
                                      input.type = "file";
                              ...
                              <td className="border border-gray-300 p-1 text-center">
                                <button
                                  onClick={() => setMultifacturasRows(prev => prev.filter((_, i) => i !== idx))}
                  ...
                  <div className="flex justify-between items-center pt-2 border-t mt-2">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setMultifacturasRows(prev => [...prev, { nro_serie: "", nro_factura: "", galones: "", proyecto: "" }])}
                      className="text-xs"
                      disabled={multifacturasLoading || multifacturasSaving}
                    >
                      + Agregar fila
                    </Button>
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        disabled={multifacturasSaving || multifacturasLoading || !multifacturasOrdenId}
                        onClick={async () => {
                          if (!multifacturasOrdenId) return;
                          setMultifacturasSaving(true);
                          try {
                            const ordenApi = multifacturasOrdenTipo === "compra" ? ordenesCompraApi : ordenesServicioApi;

                            // 1. Guardar filas de texto → obtener IDs
                            const saved = await ordenApi.saveMultifacturas(
                              multifacturasOrdenId,
                              multifacturasRows.map(r => ({
                            ...
                            // 2. Subir archivos pendientes usando los IDs recién obtenidos
                            const updatedRows = await Promise.all(
                              multifacturasRows.map(async (row, i) => {
                                ...
                                if (row.file_factura && detalleId) {
                                  ...
                                  const res = await ordenApi.uploadMultifacturaFactura(multifacturasOrdenId, detalleId, fd);
                                  url_factura = res.fileUrl;
                                }
                                if (row.file_guia && detalleId) {
                                  const fd = new FormData();
                                  fd.append("file", row.file_guia);
                                  const res = await ordenApi.uploadMultifacturaGuia(multifacturasOrdenId, detalleId, fd);
                            ...
                            setMultifacturasRows(updatedRows);
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setIsMultifacturasOpen(false)}
                        className="text-xs"
                      >
                        Cerrar
                      </Button>
