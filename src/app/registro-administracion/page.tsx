...
export default function RegistroAdministracionPage() {
  ...
  const filtrosOrdenes = useMemo(() => ({
    ...
    autoAdministrador: filtroAdministracion === "TODOS" ? undefined : filtroAdministracion === "APROBADO",
  }), [filtroEstado, fechaFiltro, filtroAdministracion]);
  ...
  const handleAprobarOrdenCompra = async (id: number) => {
    if (!confirm("¿Está seguro de que desea aprobar esta orden de compra para administración?")) {
      return;
    }
    ...
    try {
      ...
      toast.dismiss();
      toast.success("Orden de compra aprobada para administración exitosamente");
      loadOrdenesCompra();
    } catch (error) {
  ...
  const handleAprobarOrdenServicio = async (id: number) => {
    if (!confirm("¿Está seguro de que desea aprobar esta orden de servicio para administración?")) {
      return;
    }
    ...
    try {
      ...
      toast.dismiss();
      toast.success("Orden de servicio aprobada para administración exitosamente");
      loadOrdenesServicio();
    } catch (error) {
  ...
  // Búsqueda, estado, fecha y aprobación de administración ya se resuelven
                            ...
                            <div className="space-y-3 pt-2">
                              ...
                              <div className="flex items-center gap-3 pt-2 border-t">
                                <button
                                  ...
                                  title="Aprobar para Administración"
                                  disabled={!orden.id_orden_compra || orden.auto_administrador === true}
                                >
                            ...
                            <div className="space-y-3 pt-2">
                              ...
                              <div className="flex items-center gap-3 pt-2 border-t">
                                <button
                                  ...
                                  className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm text-white bg-green-600 hover:bg-green-700 rounded-lg transition-colors font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
                                  title="Aprobar para Administración"
                                  disabled={!orden.id_orden_servicio || orden.auto_administrador === true}
                                >
                                  <CheckCircle className="h-4 w-4" />
                                  Aprobar
                                </button>
